...
func (r *RedisCache) GetStats(field string) (value string, err error) {
	str, keyStats := r.client.HGet(context.Background(), r.keyStats, field).Result()
	return str, keyStats
}
...
func (r *RedisCache) SaveExecutionPayloadHeader(slot uint64, proposerPubkey, blockHash string, resp *relayCommon.GetPayloadHeaderResponse) (err error) {
	key := r.keyCacheGetPayloadResponse(slot, proposerPubkey, blockHash)
	return r.SetObj(key, resp, expiryBidCache)
}
...
func (r *RedisCache) GetExecutionPayloadHeader(slot uint64, proposerPubkey, blockHash string) (*relayCommon.GetPayloadHeaderResponse, error) {
	key := r.keyCacheGetPayloadResponse(slot, proposerPubkey, blockHash)
	resp := new(relayCommon.GetPayloadHeaderResponse)
	err := r.GetObj(key, resp)
	if errors.Is(err, redis.Nil) {
		return nil, nil
	}
...
func (r *RedisCache) SaveLatestBuilderBid(slot uint64, builderPubkey, parentHash, proposerPubkey string, receivedAt time.Time, headerResp *relayCommon.GetHeaderResponse) (err error) {
...
func (r *RedisCache) UpdateTopBid(slot uint64, parentHash, proposerPubkey string) (err error) {
	...
	for builderPubkey, bidValue := range bidValueMap {
		bidValueInt, _ := hexutil.DecodeUint64(bidValue)
		val := uint256.NewInt(bidValueInt)
